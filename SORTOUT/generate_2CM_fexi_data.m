%==========================================================================
%
% Generate MRI signals using the 2 compartment FEXI modelmeasurement_to_dependents
%
% Use: [train_inp, val_inp, test_inp, train_tar, val_tar, test_tar, measurement_to_dependents] = generate_2CM_fexi_data(n_train, n_valid, n_test)
%
...
%
% Output: 	- train_inp:  single, signals for training dataset, [n_train, n_acq]
%           - val_inp:  single, signals for validation dataset, [n_valid, n_acq]
%           - test_inp:   single, signals for testing dataset, [n_test, n_acq]
%           - train_tar:  single, model parameters a training dataset, [n_train, 4]
%           - val_tar:  single, model parameters for validation dataset, [n_train, 4]
%           - test_tar:   single, model parameters for testing dataset, [n_train, 4]
%           - measurements_to_dependents:      container map, indicates dependent acquisitions for each b==0
%
% Author: Elizabeth Powell, 02/12/2022
%
%==========================================================================

function [train_inp, val_inp, test_inp, train_tar, val_tar, test_tar, measurement_to_dependents] = generate_2CM_fexi_data(n_train, n_valid, n_test)
    ...
    train_inp = zeros(n_train,n_acq,'single');
    for i = 1:n_train
        train_inp(i,:) = s(fatm(faeq_train(i),fa0(faeq_train(i),da_train(i),db_train(i),bf),k_train(i),tm),da_train(i),db_train(i),b);
    end
    ...
    val_inp = zeros(n_valid,n_acq,'single');
    for i = 1:n_valid
        val_inp(i,:) = s(fatm(faeq_valid(i),fa0(faeq_valid(i),da_valid(i),db_valid(i),bf),k_valid(i),tm),da_valid(i),db_valid(i),b);
    end
    ...
    test_inp = zeros(n_test,n_acq,'single');
    for i = 1:n_test
        test_inp(i,:) = s(fatm(faeq_test(i),fa0(faeq_test(i),da_test(i),db_test(i),bf),k_test(i),tm),da_test(i),db_test(i),b);
    end
    ...
    train_tar = single([da_train, db_train, faeq_train, k_train]) .* [1e9,1e9,1,1];
    val_tar = single([da_valid, db_valid, faeq_valid, k_valid]) .* [1e9,1e9,1,1];
    test_tar = single([da_test, db_test, faeq_test, k_test]) .* [1e9,1e9,1,1];
    ...
    measurement_to_dependents = containers.Map(keys,values);
